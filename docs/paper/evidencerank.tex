...
{Angelica: EvidenceRank: An Evidence-Grounded LLM Pipeline for IT Candidate Ranking}
...
prompt and Pydantic-validated output schema. A shared all-MiniLM-L6-v2 singleton provides embeddings to the post-hoc
...
\node[box, below=of p2] (prefilter) {Pre-Filter\\Exact-Match\\Skill Filter};
...
\begin{enumerate}
  ...
  \item \textbf{Inferred}: not verbatim but semantically grounded—the maximum
        cosine similarity between the all-MiniLM-L6-v2 embedding of the quote
        and any individual sentence in the CV text exceeds threshold $\phi$
        (default 0.85):
        \begin{equation}
          \max_{s_j \in \mathcal{S}(\text{cv})}
            \cos\bigl(\mathbf{e}_{\text{quote}},\; \mathbf{e}_{s_j}\bigr) > \phi
          \label{eq:hallucination_threshold}
        \end{equation}
        where $\mathcal{S}(\text{cv})$ is the set of sentences produced by
        splitting the raw CV text on sentence-boundary punctuation and newlines.
        Sentence-level comparison is used rather than a single full-document
        vector because encoding a complete CV as one embedding produces a diffuse
...
\texttt{BORDERLINE\_SCORE\_THRESHOLD} (default 5.0). The hallucination similarity
threshold $\phi$ is fixed at 0.85 in the checker module. The pipeline runs entirely
...
hallucination-check embeddings, and Pydantic v2 for schema validation. Small-model tasks (Phases 1 and 2) use \texttt{qwen2.5:7b}; large-model
...
\textbf{N2 — LLM-Normalised Exact-Match Skill Pre-Filter (G3):}
...
\begin{enumerate}
  ...
  \item \textbf{Graph-augmented skill matching}: Replacing the flat exact-match
        skill pre-filter with a skill ontology graph (e.g., ESCO, O*NET as in
